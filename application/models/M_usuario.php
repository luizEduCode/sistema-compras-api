...
class M_usuario extends CI_Model {
	public function inserir($usuario, $senha, $nome, $tipo_usuario, $usuarioLogin){
		try{
			...
			if($this->db->affected_rows() > 0){
				//Usuario na base local inseriu corretamente
				//Instância do banco.
				$this->load->model('M_log');

				//Fazemos a chamada do método de inserção
				...
				if($retornoLog['codigo'] == 1){
					...
					$dados = array('codigo' => 8,
								'msg' => 'Houve algum problema no salvamento do log, porém, usuário cadastrado corretamente.');
				}
	...
	public function alterar($usuario, $nome, $senha, $tipo_usuario, $usuarioLogin){
		...
		try{
			...
			if($this->db->affected_rows() > 0){
				...
				$retornoLog = $this->M_log->inserirLog($usuarioLogin, $queryFinal);
			...
			if($retornoAdmin == 1){
				...
				if($retornoUsuario['codigo'] == 1){
					...
					if($this->db->affected_rows() > 0){
						...
						$retornoLog = $this->M_log->inserirLog($usuarioLogin, "update usuarios set estatus = 'D'
																			  where usuario = '$usuario'");
